...
def start_game(screen, gesture_recognizer, menu):
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if "gravity" in self.dippid.get_capabilities():
                main_character.change_movement(angle=self.dippid.get_value('gravity')['x'])
            # TODO: check if "angle" is bugged on m5stack, since the values seemed strange
            elif "angle" in self.dippid.get_capabilities():
                main_character.change_movement(angle=self.dippid.get_value('angle')['x'])
